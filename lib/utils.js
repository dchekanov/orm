...
function addIdExtenders({model, references, dbModels}) {
  ...
  relatedReferences.forEach(reference => {
    ...
    model.extenders[property] = async (instances, extend, ctx) => {
      ...
      const referencedInstances = await referencedModel.find({where: {id: {$in: ids}}, extend}, ctx);
...
function addIsReferencedExtender({model, references}) {
  ...
  model.extenders.isReferenced = async (instances, extend, ctx) => {
    if (!instances || instances.length === 0) {
      return;
    }
