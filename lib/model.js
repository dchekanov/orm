...
class Model {
  ...
  static extend(instances, options) {
    ...
    const properties = options.extend;

    if (!Array.isArray(instances) || instances.length === 0 || !properties || properties.length === 0) {
      return Promise.resolve(instances);
    }

    const extend = typeof properties === 'string' ? [properties] : properties;
    const extended = [];

    extend.forEach(k => {
      const extender = this.extenders[k];

      if (extender) extended.push(extender(instances, options));
    });
  ...
  static find(where, options = {}) {
    ...
    return db.query(query, options).then(r => {
      ...
      return this.extend(instances, options);
    });
  }
