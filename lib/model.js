...
class Model {
   ...
   * @returns {Promise}
   */
  static refreshColumns(execOpts = {}) {
    const spec = {
      type: 'select',
    ...
    return this.db.exec(spec, execOpts).then(result => {
   ...
   * @returns {Promise}
   */
  static async extend(instances, properties, execOpts = {}) {
    if (!execOpts.op) {
      execOpts.op = 'extend';
    }
    ...
    for (const property of properties) {
      ...
      await extender(instances, extenderExecOptions);
    }
  ...
  async save() {
    ...
    if (!this.constructor.columns) {
      await this.constructor.refreshColumns(execOpts);
    }
